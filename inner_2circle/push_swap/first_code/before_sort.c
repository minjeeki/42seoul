...
int     ft_check_duplicate_and_normalize(int argc, int *origin_arr, int *sorted_arr);
...
int ft_check_validate_and_normalize(int argc, char *argv[], int *origin_arr)
{
    ...
    if (ft_check_duplicate_and_normalize(argc, origin_arr, sorted_arr) == 1)
...
int ft_safe_atoi(char *str, int *arr_mem)
{
    ...
    while (str[idx] != '\0')
    {
        ...
        converted_num = converted_num * 10 + (str[idx] - '0');
        idx++;
    }
    converted_num *= sign;
    if (converted_num < ((long long)INT_MIN) || converted_num > ((long long)INT_MAX))
        return (ft_print_error());
    *arr_mem = (int)converted_num;
    return (0);
}
